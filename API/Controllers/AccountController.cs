    ...
    IMapper mapper) : BaseApiController
{
    ...
    public async Task<ActionResult<UserResponse>> LoginAsync(LoginRequest request)
    {
        var user = await userManager.Users
            .Include(x => x.Photos)
            .FirstOrDefaultAsync(x => x.UserName.Equals(request.Username, StringComparison.InvariantCultureIgnoreCase));
    ...
    private async Task<bool> UserExistsAsync(string username) =>
        await userManager.Users.AnyAsync(u => u.UserName == username.ToUpperInvariant());
}